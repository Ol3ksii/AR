...
export default function PostItem({ post, onInteract, isActive }) {
  ...
  const calculateReward = (explicitAction = null) => {
    if (explicitAction === 'DISLIKE') return -2.0;
    ...
    const baseReward = Math.log(1 + equivalentVisits);
    let finalReward = Math.pow(baseReward, 2);

    // Zero-Reward Grace Period (Filters out instant scrolling)
    if (watchedSeconds < 1.5 && explicitAction !== 'LIKE') {
      finalReward = 0.0;
    }

    return parseFloat(finalReward.toFixed(2));
  };
